...
export class DataLab {
  ...
  readAnalysisParameters():AnalysisParameters{
    ...
    return {
      ...
      responseFormat:(document.querySelector<HTMLSelectElement>("#lab-response-format")?.value||"both") as "bullets"|"paragraphs"|"both"
    };
  }
  private render(){
  ...
  private statistics(){
    ...
    return '<div class="stats-box">'+
      ...
      '<section class="conclusion-controls"><h3>Conclusion controls</h3><p class="muted">Choose factors you especially want conclusions about. The engine will also scan remaining eligible factors when enabled.</p><label>Specific research question<textarea id="lab-question" rows="3" placeholder="Example: Which factors are most strongly associated with revenue, and are those relationships robust?"></textarea></label><label>Prioritized factors<select id="lab-focus" multiple size="7">'+focusOpts+'</select></label><label>Answer format<select id="lab-response-format"><option value="bullets">Bullet points</option><option value="paragraphs">Paragraphs</option><option value="both" selected>Both</option></select></label><div class="parameter-grid"><label class="checkbox"><input id="lab-scan-other" type="checkbox" checked> Scan other factors automatically</label><label>Conclusion depth<select id="lab-depth"><option value="standard">Standard</option><option value="deep" selected>Deep</option><option value="research">Research</option></select></label><label>Robustness resamples<select id="lab-resamples"><option value="100">100 · Faster</option><option value="300" selected>300 · Balanced</option><option value="600">600 · Deep</option><option value="1000">1000 · Research</option></select></label></div></section>'+
